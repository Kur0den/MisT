    ...
    def _note_reload(self):
        ...
        if len(self.msk_.notes) == 0:
            ...
            if noteval["renoteId"] is not None:
                self._noteput(
                    f"{noteval['user']['name']} [{username}] was renoted",
                    "-" * (self.screen.width - 8),
                )
            else:
                self._noteput(
                    f"{noteval['user']['name']} [{username}] was noted",
                    "-" * (self.screen.width - 8),
                )
            ...
            self._noteput(
                f'{noteval["renoteCount"]} renotes, {noteval["repliesCount"]} replys',
            )
            self.buttons[3].disabled = False
    ...
    def poptheme(self):
        self._scene.add_effect(
            PopUpDialog(
                self.screen,
                "Change Theme",
                ["default", "monochrome", "green", "bright"],
