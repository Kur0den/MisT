...
class MkAPIs:
    ...
    def reload(self):
        ...
        try:
            ...
            exceptions.MisskeyAPIException,
    ...
    def get_note(self):
        try:
            if self.tl == "HTL":
                ...
                self.notes = self.mk.notes_hybrid_timeline(
                    self.tl_len, with_files=False
                )
            elif self.tl == "GTL":
                self.notes = self.mk.notes_global_timeline(
                    self.tl_len, with_files=False
                )
...
class NoteView(Frame):
    def __init__(self, screen, msk):
        ...
        self.buttons = [
            Button(buttonnames[i], on_click[i]) for i in range(len(buttonnames))
        ]
    ...
    def noteupdate(self):
        ...
        if is_ok:
            ...
            self._scene.add_effect(
                PopUpDialog(self.screen, "something occured", ["ok"])
            )
    ...
    def _note_reload(self):
        ...
        if (self.msk_.nowpoint == len(self.msk_.notes) - 1) or (
            self.msk_.nowpoint == len(self.msk_.notes)
        ):
    ...
    def _note_inp(self, note):
        ...
        if note["renoteId"] is not None:
            self._noteput(
                f"{name} [{username}] was renoted    noteId:{note['id']}",
                "-" * (self.screen.width - 8),
            )
        else:
            self._noteput(
                f"{name} [{username}] was noted    noteId:{note['id']}",
                "-" * (self.screen.width - 8),
            )
        ...
        self._noteput(
            f'{note["renoteCount"]} renotes {note["repliesCount"]} replys {sum(note["reactions"].values())} reactions',
            "  ".join(
                f'{i.replace("@.","")}[{note["reactions"][i]}]'
                for i in note["reactions"].keys()
            ),
        )
    ...
    def pop_more(self):
        self._scene.add_effect(
            PopUpDialog(
                self.screen,
                "?",
                ["Create Note", "Renote", "Reaction", "return"],
                on_close=self._ser_more,
            )
        )

    def pop_quit(self):
        self._scene.add_effect(
            PopUpDialog(self.screen, "Quit?", ["yes", "no"], on_close=self._ser_quit)
        )
    ...
    def _ser_more(self, arg):
        if arg == 0:
            ...
            if len(self.msk_.notes) == 0:
                self._scene.add_effect(
                    PopUpDialog(self.screen, "Please Note Get", ["Ok"])
                )
            else:
                if self.msk_.notes[self.msk_.nowpoint].get("renote"):
                ...
                self._scene.add_effect(
                    PopUpDialog(
                        ...
                        f"Renote this?\nnoteId:{noteid}\nname:{username}\ntext:{text}",
                        ["Ok", "No"],
                        on_close=self._ser_renote,
                    )
                )
        elif arg == 2:
            # Reaction
            self._scene.add_effect(
                PopUpDialog(self.screen, "this is not working :(", ["Ok"])
            )
    ...
    def _ser_renote(self, arg):
        if arg == 0:
            ...
            if createnote is not None:
                self._scene.add_effect(
                    PopUpDialog(self.screen, "Create success! :)", ["Ok"])
                )
            else:
                self._scene.add_effect(
                    PopUpDialog(self.screen, "Create fail :(", ["Ok"])
                )
...
class ConfigMenu(Frame):
    def __init__(self, screen, msk):
        ...
        self.buttons = [
            Button(buttonnames[i], onclicks[i]) for i in range(len(buttonnames))
        ]
...
M       M  I  SSS  T """
        ...
        version = "v0.1.0"
        self._txtbxput(
            mist_figs + version, "", "write by 35enidoi", "@iodine53@misskey.io", ""
        )
    ...
    def poptheme(self):
        self._scene.add_effect(
            PopUpDialog(
                self.screen,
                "Change Theme",
                ["default", "monochrome", "green", "bright", "return"],
                on_close=self._ser_theme,
            )
        )
    ...
    def poptoken(self):
        self._scene.add_effect(
            PopUpDialog(
                self.screen, "How to?", ["MiAuth", "TOKEN", "return"], self._ser_token
            )
        )
            ...
            if self.msk_.i is not None:
                self._scene.add_effect(
                    PopUpDialog(
                        ...
                        ["Ok", "No"],
                        on_close=self.instance_,
                    )
                )
            else:
                self._ok_value = "INSTANCE"
                self._txtbxput(
                    "input instance such as 'misskey.io' 'misskey.backspace.fm'",
                    f"current instance:{self.msk_.instance}",
                    "",
                )
                self.txt.disabled = False
                for i in self.buttons:
...
class CreateNote(Frame):
    def __init__(self, screen, msk):
        super(CreateNote, self).__init__(
            screen,
            screen.height,
            screen.width,
            title="CreateNote",
        ...
        # txtbox create
        self.txtbx = TextBox(
            screen.height - 3, as_string=True, line_wrap=True, on_change=self.reminder
        )
    ...
    def _ser_createnote(self, arg):
        if arg == 0:
            ...
            if return_ is not None:
                ...
                self._scene.add_effect(
                    PopUpDialog(self.screen, "Create note fail :(", ["Ok"])
                )
...
def wrap(screen, scene):
    scenes = [
        Scene([NoteView(screen, msk)], -1, name="Main"),
